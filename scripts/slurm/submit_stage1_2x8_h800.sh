...
PREV_CKPT=${PREV_CKPT:-$(ls -1 artifacts/train/stage0_naive/checkpoints/global_step_*.pt 2>/dev/null | tail -n 1 || true)}
if [[ -z "$PREV_CKPT" ]]; then
  echo "missing stage0 checkpoint; set PREV_CKPT=/path/to/ckpt.pt" >&2
  exit 2
fi

BASE_CFG=${BASE_CFG:-configs/train/stage1_align.yaml}
RUN_CFG=${TMP_CFG_DIR}/stage1_align.${SLURM_JOB_ID}.yaml
