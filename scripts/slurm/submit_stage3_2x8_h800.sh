...
PREV_CKPT=${PREV_CKPT:-$(ls -1 artifacts/train/stage2_full/checkpoints/global_step_*.pt 2>/dev/null | tail -n 1 || true)}
if [[ -z "$PREV_CKPT" ]]; then
  echo "missing stage2 checkpoint; set PREV_CKPT=/path/to/ckpt.pt" >&2
  exit 2
fi

BASE_CFG=${BASE_CFG:-configs/train/stage3_lownfe.yaml}
RUN_CFG=${TMP_CFG_DIR}/stage3_lownfe.${SLURM_JOB_ID}.yaml
